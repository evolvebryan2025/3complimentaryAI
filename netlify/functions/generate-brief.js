...
function composeEmail(meetingBriefs, user) {
    ...
    <div style="margin-bottom: 32px;">
      <div style="display:flex;align-items:center;margin-bottom:16px;">
        <div style="background:linear-gradient(135deg,#667eea,#764ba2);color:white;width:40px;height:40px;border-radius:10px;display:inline-block;text-align:center;line-height:40px;font-size:18px;font-weight:700;margin-right:14px;flex-shrink:0;">${i + 1}</div>
        <div>
          <div style="font-size:18px;font-weight:700;color:#1a1a2e;margin-bottom:2px;">${b.subject}</div>
          <div style="font-size:13px;color:#888;">
            ${fmtTime(b.start_time)}${duration ? ` · ${duration}` : ''}${attendeeNames ? ` · ${attendeeNames}` : ''}
          </div>
        </div>
      </div>
      ${hangout || location ? `<div style="margin-bottom:14px;font-size:13px;">${hangout ? `<a href="${hangout}" style="color:#667eea;text-decoration:none;margin-right:16px;">🎥 Join Video Call</a>` : ''}${location ? `<span style="color:#888;">📍 ${location}</span>` : ''}</div>` : ''}
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;background:#f0f2f5;margin:0;padding:0;">
  <div style="max-width:680px;margin:0 auto;padding:24px 16px;">
    <!-- Header -->
    <div style="background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);border-radius:14px 14px 0 0;padding:28px 32px;color:white;">
      <div style="font-size:13px;text-transform:uppercase;letter-spacing:0.08em;opacity:0.8;margin-bottom:4px;">Meeting Preparation Brief</div>
    ...
    <!-- Body -->
    <div style="background:white;border-radius:0 0 14px 14px;padding:32px;box-shadow:0 2px 12px rgba(0,0,0,0.06);">
      ${meetingCards}
    </div>
    ...
    <div style="text-align:center;padding:20px 0 8px 0;">
      <div style="font-size:11px;color:#aaa;">
        Generated by Meeting Preparation Automation · Powered by <a href="https://madeeas.com" style="color:#FD5811;text-decoration:none;">MadeEA</a>
      </div>
    </div>
  </div>
</body></html>`;
